...
- [x] 7.1 **Done on 2026-08-27, and the pre-deploy probe refused for the first time.** A release with a knowingly wrong credential stopped at step 10: migrate, deploy and readiness never ran, production stayed on the commit it was running, and the run reported `production is untouched` because it was. No label landed anywhere, because the branch resolved to no ticket and the window step is gated on having one.

  **Two failed attempts first, and both taught something the design did not know.** A release dispatched with main's head being a secrets commit died at the artifact check — nothing builds an image for a commit with no pull request, so a rehearsal can never ride a wiring commit. Then a release dispatched from an older branch passed the probe with a *good* password: **the credential the probe uses comes from the commit being released, not from the current main.** That is a property with a consequence nobody had written down — rotating the smoke account's password does not affect releases of commits that predate the rotation. The rehearsal finally needed a branch carrying both the broken credential and an artifact of its own.
- [x] 7.2 **Restored, and verified two ways rather than by the edit.** The secret reads back as the original and not as the rehearsal's value. And a release of a commit carrying the good credential passed **both** probes minutes earlier — the run at 16:03, `1 passed` before the deploy and `1 passed` after — so the passing direction is observed and not assumed. The restore itself reaches a release only through a commit that contains it, which is the property 7.1 uncovered
- [x] 7.3 **The price, as it actually fell.** Two reconfigure deploys of an unchanged version, as predicted. But **no `blocked:rollback` label anywhere**, which the prediction got wrong for a reason worth keeping: the rehearsal ran from a branch that resolves to no ticket, so the window step was skipped entirely. The predicted cleanup was unnecessary. Also unpredicted: one wasted release attempt that died at the artifact check, and one that passed for the wrong reason — cheap, and both were findings
