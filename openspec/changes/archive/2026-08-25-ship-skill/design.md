...
- **The invocation budget is advisory, not enforced** → It is written as a rule the agent consults, and nothing outside the agent counts invocations or applies the label. That is weaker than the design of it implied: an agent looping is precisely the one that will not stop to check its own budget, so a self-enforced bound does not bound the case it exists for. Enforcement belongs in whatever invokes the agent — it can count and refuse — and until it is there, the budget bounds honest mistakes rather than runaway ones. Recorded rather than closed, because a rule that reads as a safeguard while enforcing nothing is the failure this project keeps finding.
