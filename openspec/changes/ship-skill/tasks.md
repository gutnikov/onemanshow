...
- [ ] 6.5 Enforce the six merge guards in the pipeline rather than in prose. **Not enforced as built.** All six are readable from the tools and the run of 2026-08-25 satisfied all six, but nothing checked them - the merge is a fast-forward push that no workflow inspects, so the guards hold exactly as long as whoever merges reads them. Same shape as 6.3: the actor being constrained is the one asked to apply the constraint. Belongs with the automation change, which is the only party that sees a merge it did not make
- [x] 6.6 Verify a rollback is possible rather than assumed: confirm the previous production image is still in the registry during a live observation window. Done in the same run - `f9436638…-production` was present, so the safety net the window depends on existed in fact and not only in the design
