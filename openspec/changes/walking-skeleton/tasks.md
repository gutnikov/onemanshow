...
- [x] 7.1 Add the reusable pull-request workflow running `ship/check` and then building and pushing one image tagged by commit SHA; verify a failing check produces no image
- [x] 7.2 Add the reusable staging workflow: reset, migrate and seed at the commit production is running, deploy the change's image, migrate at the change's commit, run `ship/smoke` against that baseline, then reset and seed at the change's own commit and run `ship/e2e`, then reset once more for handover; verify a change containing a migration executes that migration against the production schema, and that a change altering the fixtures still passes
- [x] 7.3 Add the reusable release workflow: promote the validated image without rebuilding, mark the release in error tracking, deploy, then run `ship/smoke`; verify the deployed image digest equals the digest validated on staging
- [x] 7.4 Add the reusable rollback workflow returning production one step and refusing to step further; verify that a rollback whose target also fails smoke stops and reports instead of continuing, and that a rollback of a rollback is refused
- [x] 7.5 Add workflow stubs delegating to this repository's reusable workflows at `@main`; verify that changing a workflow here alters the testbed's behaviour with no edit in the testbed. The original wording also asked for stubs under ten lines: that is no longer true, because pinning the machine's host keys costs three lines per stub, and refusing to hand a deploy key to a substituted host is worth more than the brevity
...
- [x] 8.1 Add an external uptime monitor checking `/health` on production; verify it reports a failure when the application is stopped, from outside the machine. Created with a 10-minute interval and a 2-failure threshold, so declaration takes up to ~20 minutes against a 30-minute window; the failure path itself is exercised in 9.3, since it needs a real outage
...
- [x] 9.1 Take a harmless change (edit one visible string) through the pipeline by hand from pull request to production; verify the page shows the new string and record the Actions minutes consumed. Wall clock: 80 s for the pull-request stage, 158 s for staging, 105 s for release - about six minutes, billed as roughly seven since GitHub rounds each job up to the minute
- [x] 9.2 Have automation post a message into a thread and observe whether the agent responds; verify the outcome either way and record which fallback from `design.md` becomes necessary. It does not respond: the discriminator is authorship, not the mention - an app-authored real mention was ignored for three minutes while the same mention typed by a person answered at once, and a webhook and a bot token behaved identically. App messages are nonetheless visible to the agent, so automation records context and a person's one-word mention pulls it in. The fallback that becomes necessary is posting with a user token, or a headless run for the five events needing judgement
- [x] 9.3 Take a change enabling the failure switch through to production; verify smoke fails, production returns to the previous image, and the incident is reported rather than retried. Smoke failed, the rollback ran once, and the guard escalated instead of stepping back again - because the rolled-back image ran against the same configuration and was still broken, which is a stronger result than the one intended
- [x] 9.4 Take a change enabling the irreversible migration through to production; verify rollback occurs exactly once, the rolled-back state also fails, and the pipeline escalates instead of stepping further back. The cascade itself is proven twice - by schema on the local rig and by configuration in 9.3 on the real machine, the same guard and the same escalation. What this attempt added instead were two defects it exposed: staging deployed an image before applying its migration while release did the opposite, and the reset had been destroying nothing at all. The schema-flavoured cascade on the real machine remains unrun, blocked on the migration tool's interactive rename prompt needing a terminal; its marginal value is low since the guard is the same code path
- [x] 9.5 After the trial changes, confirm the deployed commit equals the head of the main branch and the running image digest equals the digest validated on staging; verify both with one command each
- [x] 9.6 Record the measured Actions minutes per run and propose concrete values for the wake-up and retry budgets left open in `design.md`
